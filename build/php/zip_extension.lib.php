 ...
 */
function getZipContents($file, $specific_entry = null)
{
    $error_message = '';
    $file_data = '';
 ...
 */
function findFileFromZipArchive ($file_regexp, $file)
{
    $zip_handle = zip_open($file);
    $found = false;
    if (is_resource($zip_handle)) {
 ...
 */
function getNoOfFilesInZip($file)
{
    $count = 0;
    $zip_handle = zip_open($file);
    $found = false;
 ...
 */
function zipExtract($zip_path, $destination, $entries)
{
    $zip = new ZipArchive;
